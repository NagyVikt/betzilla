 ...
 */
export default function ScrollProgressHeader() {
  ...
  useEffect(() => {
    const calculateHeaderHeight = () => {
      ...
      const mainHeaderEl = document.querySelector('header[role="banner"], body > header');
  ...
  useEffect(() => {
    const handleScroll = throttle(() => {
      ...
      setProgress(totalScrollableHeight > 0 ? (scrollTop / totalScrollableHeight) * 100 : 0);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (componentHeaderRef.current && !componentHeaderRef.current.contains(event.target as Node)) {
        setIsExpanded(false);
      }
    };
  ...
  const updateCurrentSection = (scrollTop: number) => {
    ...
    const activeSection = sectionsRef.current.find((section, index) => {
      ...
      return scrollOffset >= section.top && (!nextSection || scrollOffset < nextSection.top);
    });
  ...
  const handleGoToSection = (id: string) => {
    ...
    if (sectionElement) {
        ...
        const offsetPosition = elementPosition + window.scrollY - getTotalStickyOffset() - 16; // Extra margin
    ...
    >
      <div className="relative w-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm shadow-sm">
        ...
        >
          ...
          <svg className={clsx("w-5 h-5 text-gray-500 dark:text-gray-400 transition-transform duration-300 ease-in-out", isExpanded && "rotate-180")} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          </svg>
        </div>
        
        <div className="w-full h-1">
          <div className="h-1 bg-gradient-to-r from-pink-400 to-rose-500 transition-[width] duration-150 ease-linear" style={{ width: `${progress}%` }}/>
        </div>

        <div className={clsx("absolute top-full right-4 mt-2 w-72 origin-top-right transition-all duration-200 ease-out", isExpanded ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none')}>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <ol className="divide-y divide-gray-200 dark:divide-gray-700">
                  {sections.map((section, idx) => (
                    <li key={section.id}>
                      <button onClick={() => handleGoToSection(section.id)} className={clsx("w-full flex items-center gap-4 px-4 py-3 text-left transition-colors duration-150", section.id === currentSectionId ? "bg-pink-50 dark:bg-pink-900/40" : "hover:bg-gray-100 dark:hover:bg-gray-700/50")}>
                        <span className={clsx("text-xs font-bold w-5 text-center", section.id === currentSectionId ? "text-pink-600 dark:text-pink-400" : "text-gray-400 dark:text-gray-500")}>{idx + 1}</span>
                        <span className={clsx("flex-1 text-sm font-medium", section.id === currentSectionId ? "text-pink-700 dark:text-pink-300" : "text-gray-800 dark:text-gray-200")}>{section.label}</span>
                      </button>
                    </li>
                  ))}
                </ol>
