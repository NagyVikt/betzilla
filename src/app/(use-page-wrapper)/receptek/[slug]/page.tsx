...
// Our rating & chart components
import { StarRating } from "@/components/Elements/StarRating";
import { NutritionChart } from "@/components/Elements/NutritionChart";

// The new client component for TOC bubbles
import TocSidebar from "@/components/Receptek/TocSidebar";

// Define the Ingredient type for type-checking in the sidebar panel
...
// Define the PreparationStep type for the new visual steps
 ...
 * Fetch all existing recipe slugs at build time so Next.js knows which pages to pre-render.
 */
...
/**
 * 2) generateMetadata:
 * For each recipe slug, fetch the blog from Strapi and return metadata (openGraph, twitter, etc.).
 */
export async function generateMetadata({
  ...
  const imageUrl =
    blog.banner?.src ||
    blog.image?.src ||
    blog.banner?.data?.attributes?.url ||
 ...
 * - Fetches the recipe by slug from Strapi at request time.
 * - If not found, calls `notFound()` → 404.
 * - Otherwise, renders:
 * • Banner (hero) with Tag, title, description, star rating, average label  
 * • BlogDetails (publikálva, szerző, olvasási idő, megtekintés)  
 * • Mobile “bubble” TOC on left via <TocSidebar />  
 * • Desktop TOC on left (Összefoglaló, Hozzávalók, etc.)  
 * • Center content (Leírás, Összefoglaló, Hozzávalók, Elkészítés, Tápérték)  
...
export default async function ReceptekPage({
  ...
  // ——————————————————————————————
  // Helper: Extract plain text for JSON-LD
  // ——————————————————————————————
  ...
  const jsonLd = {
    ...
    "@type": "Recipe",
    name: blog.title,
    author: { "@type": "Person", name: blog.Author || "Ismeretlen szerző" },
  ...
  // Data for the ingredients panel from the user-provided table
  // I've added 'store' and 'onSale' fields to demonstrate the new feature.
  const ingredientsData: Ingredient[] = [
    { category: "A tésztához:", name: "Finomliszt", quantity: "25", unit: "dkg", store: "Lidl", onSale: true },
    { category: "A tésztához:", name: "Rama (vagy vaj)", quantity: "12", unit: "dkg", store: "Tesco" },
    { category: "A tésztához:", name: "Cukor", quantity: "5", unit: "dkg", store: "Kaufland", onSale: true },
    { category: "A tésztához:", name: "Só", quantity: "1", unit: "csipet" },
    { category: "A tésztához:", name: "Sütőpor", quantity: "1", unit: "teáskanál" },
    { category: "A tésztához:", name: "Citromhéj (reszelt)", quantity: "1", unit: "db" },
    { category: "A töltelékhez:", name: "Alma (héjától megszabadítva, szeletelve)", quantity: "1,2", unit: "kg", store: "Lidl" },
    { category: "A töltelékhez:", name: "Cukor", quantity: "10", unit: "dkg", store: "Kaufland" },
    { category: "A töltelékhez:", name: "Fahéj", quantity: "1", unit: "teáskanál" },
    { category: "A tetejére:", name: "Porcukor", quantity: "2", unit: "evőkanál", store: "Tesco" },
  ];
  ...
  // ——————————————————————————————
  // Data for the visual preparation steps
  // ——————————————————————————————
  const preparationSteps: PreparationStep[] = [
    {
      stepNumber: 1,
      title: "Előkészítés",
      description: "Melegítsd elő a sütőt 180 °C-ra (légkeverés nélkül). Készíts elő egy 26 cm átmérőjű piteformát, és vékonyan kend ki olajjal vagy vajjal.",
      images: [
        blog.images?.[0]?.src || 'https://placehold.co/600x400/fecaca/312e81?text=Sütő+előmelegítése',
      ]
    },
    {
      stepNumber: 2,
      title: "Tészta elkészítése",
      description: "Egy nagy tálban keverd össze a lisztet, a sütőport, a csipet sót és az 5 dkg cukrot. Add hozzá a hideg, kis kockákra vágott Rama (vagy vajat), majd gyors mozdulatokkal morzsold el a liszttel: a cél, hogy olyan állagú legyen, mint a finomított morzsa. Egy külön kis tálban keverd össze a reszelt citromhéjat és egy evőkanál hideg vízzel, majd ezt is add a lisztes-Rama keverékhez. Gyorsan dolgozd össze a tésztát, formázz belőle félgömböt, csomagold fóliába, és tedd a hűtőbe 15–20 percre pihentetni.",
      images: [
        blog.images?.[1]?.src || 'https://placehold.co/300x200/bfdbfe/312e81?text=Száraz+hozzávalók',
        blog.images?.[2]?.src || 'https://placehold.co/300x200/ddd6fe/312e81?text=Morzsolás'
      ]
    },
    {
        stepNumber: 3,
        title: "Töltelék összeállítása",
        description: "A lehéjazott, vékonyra szeletelt almát tedd egy nagy tálba. Szórd rá a 10 dkg cukrot és a fahéjat, majd óvatosan forgasd össze, hogy minden szelet egyenletesen bevonódjon. Hagyd állni 5–10 percig, míg az alma kiengedi a levét.",
        images: [
            blog.images?.[3]?.src || 'https://placehold.co/600x400/c7d2fe/312e81?text=Almás+töltelék'
        ]
    },
    {
        stepNumber: 4,
        title: "Tészta kinyújtása és formázása",
        description: "A hűtőből kivett tésztát egy liszttel meghintett munkalapon nyújtsd ki kb. 3 mm vastag körlappá (kb. 28–30 cm átmérő). A körlap egyik felével béleld ki a piteforma alját és oldalát úgy, hogy a tészta széle kissé lelógjon.",
        images: [
            blog.images?.[4]?.src || 'https://placehold.co/600x400/fbcfe8/312e81?text=Tészta+nyújtása'
        ]
    },
    {
        stepNumber: 5,
        title: "Pite összeállítása",
        description: "Simítsd el az almás tölteléket a tésztán. Ha szeretnél rácsos tetejű pitét, vágd fel a maradék tésztalapot kb. 1–1,5 cm széles csíkokra, és rácsozd a töltelékre. Ha nem készítesz rácsot, vedd elő a másik tésztalapot, terítsd rá a töltelékre, majd a két lapot zárd le a széleken. A felső tésztára villával készíts néhány szellőző lyukat, hogy a gőz távozni tudjon sütés közben.",
        images: [
    ...
    },
    {
        stepNumber: 6,
        title: "Sütés",
        description: "Tedd a piteformát a sütő középső rácsára, és süsd 45 percig, vagy amíg a teteje aranybarna nem lesz, és a töltelék enyhén bugyogni nem kezd a lyukaknál. Ha a teteje nagyon gyorsan pirulna, takard le vékony alufóliával az utolsó 10 percben.",
        images: [
            blog.images?.[5]?.src || 'https://placehold.co/600x400/fef08a/312e81?text=Sütés'
        ]
    },
    {
        stepNumber: 7,
        title: "Kihűlés és tálalás",
        description: "A sütőből kivéve hagyd a pitét a formában 10–15 percre hűlni, hogy a töltelék kissé megszilárduljon. Ezután szórd meg a tetejét a 2 evőkanál porcukorral, szeleteld fel 8 egyenlő darabra, és tálald langyosan, például vaníliafagylalttal vagy tejszínhabbal.",
        images: [
            blog.images?.[6]?.src || 'https://placehold.co/600x400/d9f99d/312e81?text=Tálalás'
        ]
    }
  ];
  
  // ——————————————————————————————
  // Macronutrient calculations (for chart + bars)
  // ——————————————————————————————
  const nutritionValues = {
    protein: blog.protein || 0,
    carbs: blog.carbs || 0,
    fat: blog.fat || 0,
  };
  const totalMacros =
  ...
  const bannerImageUrl =
    blog.banner?.src || blog.banner?.data?.attributes?.url || "";
  const fallbackBannerColor = "bg-pink-400";
  
  let totalTimeValue: number | null = null;
  if (typeof blog.totalTime === "number") {
    totalTimeValue = blog.totalTime;
  } else if (blog.prepTime != null && blog.cookTime != null) {
    totalTimeValue = blog.prepTime + blog.cookTime;
  }
  // ——————————————————————————————
  // Table of Contents (TOC) items
  // ——————————————————————————————
  // NOTE: “Rövid leírás” is considered part of “Összefoglaló.” If you want a separate “R,” uncomment it below.
  const tocItems = [
    { href: "#section-leiras", label: "Leírás", dataField: blog.content },
    { href: "#section-osszefoglalo", label: "Összefoglaló", dataField: true },
    // { href: "#section-rovid", label: "Rövid leírás", dataField: blog.summary },
  ...
  const getStoreLogoUrl = (storeName: string): string => {
      ...
      // Fallback for stores not in the map
      return storeLogos[storeName] || `https://placehold.co/80x40/cccccc/000000?text=${storeName}`;
  };
  ...
  return (
    <>
      ...
      {/* ====================================================== */}
      {/* Mobile: Animated “bubble” TOC on left (via TocSidebar) */}
      {/* ====================================================== */}
      <TocSidebar
        items={tocItems}
      ...
      <ScrollProgressHeader slug={params.slug} />

      {/* We add left‐padding (“pl-12”) on all screen sizes so the bubbles have their own column.
          On large (lg) screens, the TocSidebar is hidden (display: none), so the extra padding doesn’t matter.
      */}
      <main className=" mx-auto mt-10 md:mt-10 px-0 sm:px-6 lg:px-8  lg:pl-8">
        ...
        <div className="relative h-[50vh] sm:h-[60vh] md:h-[70vh] w-full md:rounded-xl overflow-hidden shadow-2xl mb-8">
          {/* If a banner image exists, render it with next/image; otherwise fallback to pink background */}
          {bannerImageUrl ? (
            <Image
              src={bannerImageUrl}
          ...
          {/* Centered text content (tag, title, description, star rating) */}
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 md:p-12 text-center">
            <div className="max-w-3xl">
              {/* If there’s at least one tag, show it as a pill */}
              {blog.tags?.[0] && (
                <Tag
        ...
        {/* ———————————————————————————————————————————————————————————————————————— */}
        {/* 2) BlogDetails Section (Publikálva, Szerző, Olvasási idő, Megtekintés)        */}
        {/* ———————————————————————————————————————————————————————————————————————— */}
        ...
        {/* —————————————————————————————————————————————————————————————————————————————————————— */}
        {/* 3) Grid Layout: Desktop TOC Sidebar (lg+), Center Content, Right Gallery          */}
        {/* —————————————————————————————————————————————————————————————————————————————————————— */}
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-12 gap-8 px-4 sm:px-0">
                      ...
                      <ul className="space-y-3">
                        {items.map((ing, idx) => (
                          <li key={idx} className="flex items-center text-gray-700">
                            <input type="checkbox" className="h-5 w-5 rounded border-gray-300 text-pink-600 focus:ring-pink-500 mr-3 shrink-0" />
                            ...
                            <div className="flex items-center shrink-0 ml-2">
                              ...
                              {ing.onSale && (
                                <span className="relative inline-flex items-center justify-center w-6 h-6 bg-red-500 text-white font-bold text-xs rounded-full ml-2" title="Akciós!">
                                  %
                                </span>
                              )}
                            </div>
          ...
          {/* —————————————————————————————————————————————————————————————————————————————————————— */}
          {/* Center Content Area (spans 6/12 on lg)                                                              */}
          {/* —————————————————————————————————————————————————————————————————————————————————————— */}
          <div className="lg:col-span-6 space-y-12">
            ...
            {blog.Ingredients &&
              (!Array.isArray(blog.Ingredients) ||
                ...
                >
                  ...
                  {/* Ingredients table, if provided */}
                  {blog.ingredientsTable &&
            ...
            {/* ===== Elkészítés Section (NEW VISUAL LAYOUT) ===== */}
                ...
                >
                  ...
                  <div className="space-y-12">
                      {preparationSteps.map((step) => (
                          <div key={step.stepNumber} className="flex flex-col md:flex-row gap-6 md:gap-8 border-b border-gray-200 dark:border-gray-700 pb-8 last:border-b-0 last:pb-0">
                              ...
                              <div className="md:w-3/5 flex-shrink-0">
                                  <div className="flex items-center mb-3">
                                      ...
                                      <h3 className="text-xl font-bold text-gray-900 dark:text-gray-200">{step.title}</h3>
                                  </div>
                              ...
                              <div className="md:w-2/5">
                                  <div className={`grid gap-4 ${step.images.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                                      {step.images.map((imgSrc, imgIdx) => (
                                          <div key={imgIdx} className="relative w-full aspect-square rounded-lg overflow-hidden shadow-md group">
                                              <Image
                                                  src={imgSrc}
                                       ...
                                       {/* Optional: Button to add more images by the user */}
          ...
          {/* —————————————————————————————————————————————————————————————————————————————————————— */}
          {/* Right Column: Sticky Image Gallery (spans 4/12 on lg)                                           */}
          {/* —————————————————————————————————————————————————————————————————————————————————————— */}
          <aside className="lg:col-span-3">
            {blog.images &&
              ...
              >
                ...
                >
                  {blog.images.map((img: any, idx: number) => {
                    const imgSrc = img?.src || img?.url || img?.attributes?.url;
                    if (!imgSrc) return null;
                    return (
                      <div
                        key={img?.id || img?.hash || idx}
